function [ output, grad ] = mlp(M, H1, H2, XL, XR, weights, training, ti)
...
%second layer
r2LR=r3.*W3'.*(1./(1+exp(-A2L))).*(1./(1+exp(-A2R)));
r2L= r3.*W3'.*Z2.*(1-1./(1+exp(-A2L)));
r2R= r3.*W3'.*Z2.*(1-1./(1+exp(-A2R)));
...
gp1L=1-Z1L.^2;
gp1R=1-Z1R.^2;
r1L=gp1L.*((r2L'*W2L)'+(r2LR'*W2LR(:,1:H1))');
r1R=gp1R.*((r2R'*W2R)'+(r2LR'*W2LR(:,H1+1:end))');
